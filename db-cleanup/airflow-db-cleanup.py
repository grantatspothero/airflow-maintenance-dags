...
from config import dataeng_default_args
...
# is set to 30, the job will remove those files that are 180 days old or older.
DEFAULT_MAX_DB_ENTRY_AGE_IN_DAYS = lambda: int(
    Variable.get("airflow_db_cleanup__max_db_entry_age_in_days", 180)
)
...
ENABLE_DELETE = (
    lambda: Variable.get("airflow_db_cleanup__enable_delete", "False").lower() == "true"
)
...
dag = DAG(
    DAG_ID,
    default_args={"depends_on_past": False, **dataeng_default_args},
...
def cleanup_function(**context):
    ...
    try:
        ...
        if PRINT_DELETES:
            count_of_entries_to_delete = query.count()
            logging.info(
                f"Process will delete this many entries: {count_of_entries_to_delete} from the model: {airflow_db_model.__name__}"
            )

            MAX_ROWS_TO_PRINT = 500
            entries_to_delete = None
            if count_of_entries_to_delete > MAX_ROWS_TO_PRINT:
                logging.info(
                    f"Only printing info for the first {MAX_ROWS_TO_PRINT} entries, not printing every single entry"
                )
